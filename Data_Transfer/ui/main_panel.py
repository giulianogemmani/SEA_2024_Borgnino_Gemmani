...
class MainPanel(wx.Panel): # I create a panel where I can put the outher GUI elements
    def __init__(self, parent, interface, logic):
        ...
        self.btnRunMultipleStudyCases = wx.Button(self, -1, "Run simulation(s)", size=wx.Size(200,25))  
        self.middleoptionsGridSizer.Add(self.btnRunMultipleStudyCases, 0, wx.ALIGN_CENTER_HORIZONTAL | wx.TOP , border=4)
        self.btnRunMultipleStudyCases.Bind(wx.EVT_BUTTON, self.OnRunSimulations, self.btnRunMultipleStudyCases)
...
#                     print(settings[widget.GetName()])
    ...
    def OnRunSimulations(self, event):
        ''' 
        ...
        '''
        # Get the selected source file from the file input field
    ...
    def OnMoveallpicturetoWord(self, event):
        ...
        self.Parent.parent.Hide()
    # Get the selected source file from the file input field
        source_file = self.source_file_name.GetValue()
        # Get the selected result file from the file input field
        result_file = self.results_file_name.GetValue()
    # put together all pictures
        self.logic.move_all_pictures_together(source_file, result_file)
